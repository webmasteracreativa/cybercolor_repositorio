...
( function( $ ) {
	...
	var PostsCarouselFrontEndHandler = function( $scope, $ ) {
		...
		$swiper.find('.ee-grid__item')._resize( function() {
	...
	var CircleProgressFrontEndHandler = function( $scope, $ ) {
		...
		if ( elementSettings.value ) {
			var inputValue = Math.abs( parseFloat( elementSettings.value.replace( ',', '.' ) ) );

			if ( ! isNaN( inputValue ) ) {
				inputValue = ( inputValue > 1 ) ? inputValue / 100 : inputValue;
				cpArgs.value = inputValue;
			}
		}
